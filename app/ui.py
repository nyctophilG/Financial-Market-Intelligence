...
from main import (
    process_query, save_uploaded_file,
    list_uploaded_files, is_crewai_query, get_chroma_doc_count,
)
...
.btn-ingest button:hover {
    background: #152818 !important;
...
with st.sidebar:
    ...
    if n_docs > 0:
        ...
        st.markdown('<span class="chroma-pill chroma-empty">🗄 ChromaDB: unavailable</span>', unsafe_allow_html=True)
    ...
    if uploaded:
        st.markdown(
            f'<div style="font-size:0.74rem;color:#3a5060;padding:4px 0 6px 2px;">'
            f'{len(uploaded)} file(s) staged</div>',
            unsafe_allow_html=True
        )
    ...
    if st.button("✅  Save & Ingest into ChromaDB", key="btn_ingest"):
        if uploaded:
            ...
            for i, f in enumerate(uploaded):
                prog.progress((i+1)/len(uploaded), text=f"Ingesting {f.name}…")
                _, msg = save_uploaded_file(f.read(), f.name)
                log.append(msg)
            prog.empty()
    ...
    if st.session_state.show_files:
        ...
        if idx["raw"] or idx["processed"]:
            if idx["raw"]:
                ...
                for fn in idx["raw"]:
                    st.markdown(f'<div style="font-size:0.72rem;color:#4a7aba;font-family:monospace;padding:1px 0 1px 10px;">📄 {fn}</div>', unsafe_allow_html=True)
            if idx["processed"]:
                st.caption("data/processed/  (CSV/TXT)")
                for fn in idx["processed"]:
                    st.markdown(f'<div style="font-size:0.72rem;color:#6a9ad4;font-family:monospace;padding:1px 0 1px 10px;">📄 {fn}</div>', unsafe_allow_html=True)
        else:
    ...
    <div style="font-size:0.68rem;color:#2a3a50;line-height:1.7;">
      <span style="color:#6a7aee;">●</span> <b style="color:#3a4a70;">CrewAI</b> triggers:<br>
      <span style="color:#2a3a60;">"analyze", "revenue", "from the file",<br>
      "net sales", "earnings", SEC terms,<br>
      Q1/Q2/Q3/Q4, ticker symbols…</span><br><br>
      <span style="color:#4a9a5a;">●</span> <b style="color:#3a5a40;">Ollama</b> — everything else
    </div>
    """, unsafe_allow_html=True)
...
<div style="display:flex;align-items:center;gap:12px;
  ...
  <div>
    ...
    <div style="font-size:0.65rem;color:#2a3040;font-family:'IBM Plex Mono',monospace;letter-spacing:0.06em;">
...
if not history:
    ...
    hint = f"🗄 {n} chunks in ChromaDB — ready for analysis" if n > 0 else "📂 Upload & ingest files on the left first"
    st.markdown(f"""
...
prompt = st.chat_input("Ask anything… 'analyze', 'revenue', 'from the file' → CrewAI pipeline")
